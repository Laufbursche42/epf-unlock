...
- Any settings you make on the page (speed limits, ride stage, switches). They exist only in the open page during your session.
...
When you open or reload the page, your browser fetches the static files (`index.html`, `app.js`, `protocol.js`, `i18n.js`, `styles.css` and the favicon) from the host (for example GitHub Pages). The host sees only two things: your **IP address** and which file you requested. Those are the normal web-server logs every website has. It **never** sees any scooter data or commands: not at page load, not at reload, not at any time. That data never reaches any server at all; it exists only on your device and travels only over the local Bluetooth link (see below).
...
Nothing is ever sent to the developer or to any manufacturer backend. There is no cloud account and no server operated by this project that receives your data. Analysis of the official ePowerFun app found that it, too, uses no sign-in and no server contact, its firmware ships locally inside the app. This app is just as frugal.
